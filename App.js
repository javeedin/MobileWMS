    ...
    try {
      ...
      if (response.ok && data && (data.status === 'success' || data.STATUS === 'SUCCESS' || data.items?.length > 0)) {
        ...
        try {
          ...
          // Group by warehouse, collect all subinventories (with locator_id) per warehouse
          const warehouseMap = {};
          (orgsData.items || []).forEach(item => {
            ...
            if (!warehouseMap[item.warehouse]) {
              warehouseMap[item.warehouse] = {
                ...
                subinventories: [], // [{code, locator_id}]
              };
            }
            if (item.subinventory_code) {
              const already = warehouseMap[item.warehouse].subinventories.find(s => s.code === item.subinventory_code);
              if (!already) {
                warehouseMap[item.warehouse].subinventories.push({
                  code: item.subinventory_code,
                  locator_id: item.locator_id || null,
                });
              }
            }
          });
          ...
          setOrganizations([
            { warehouse: 'AMS', warehouse_code: 'AMS', subinventories: [] },
            { warehouse: 'MLCECLAIM', warehouse_code: 'MLCECLAIM', subinventories: [] },
          ]); // fallback
        } finally {
...
_Sent from MobileWMS_`;
  ...
  const fetchLocatorData = async (orgCode) => {
    ...
    try {
      ...
      const url = `https://g827cd88c3cfc03-mitsumioracledb.adb.me-dubai-1.oraclecloudapps.com/ords/test/INVENTORY/getonhandsbylocator?P_ORGANIZATIONCODE=${orgCode}`;
  ...
  const _buildMappedLocators = (fusionItems, onhandItems) => {
    ...
    onhandItems.forEach(item => {
      ...
      if (locatorName) {
        ...
        if (!onhandByLocator[locatorName]) onhandByLocator[locatorName] = [];
        onhandByLocator[locatorName].push(item);
      }
    });
    return fusionItems.map(loc => {
  ...
  const fetchStockLocators = async (orgOverride, subOverride, onComplete) => {
    ...
    if (!fusionLocatorId) {
      Alert.alert(
        'Locator ID Missing',
        `No Locator ID configured for subinventory "${effectiveSub || 'selected'}". Please configure it in Oracle or select a different subinventory (look for ✓ in the LID column).`
      );
  ...
  const fetchMoreLocators = async () => {
    ...
    const effectiveSub = locatorSelectedSub;

    const selectedSubObj = effectiveOrg?.subinventories?.find(s => s.code === effectiveSub);
    const fusionLocatorId = selectedSubObj?.locator_id;
    if (!fusionLocatorId || locatorFetchingMore) return;

    setLocatorFetchingMore(true);
    const LIMIT = locatorPageSize;
    try {
      const fusionResponse = await fetch(
        `${ORACLE_FUSION_BASE}/subinventories/${fusionLocatorId}/child/locators?offset=${locatorFusionOffset}&limit=${LIMIT}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Basic ${ORACLE_FUSION_AUTH}`,
      ...
      console.log(`Fusion next page offset=${locatorFusionOffset}: ${pageItems.length} items, hasMore=${serverHasMore}`);

      const newFusionItems = [...fusionLocators, ...pageItems];
      setFusionLocators(newFusionItems);
      setLocatorFusionOffset(locatorFusionOffset + LIMIT);
      setLocatorApiHasMore(serverHasMore);

      const mapped = _buildMappedLocators(newFusionItems, onhandLocators);
      setMappedLocators(mapped);
    } catch (e) {
      Alert.alert('Error', 'Failed to load more locators: ' + e.message);
    } finally {
      setLocatorFetchingMore(false);
    }
  };
  ...
  const checkLocatorStatus = async (locatorName, currentLineId = null) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/getonhandsbylocator?P_ORGANIZATIONCODE=${orgCode}`);
      const data = await response.json();
      ...
      <Modal visible={showApiInfoModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.parameterModalContainer, { maxHeight: '80%' }]}>
            ...
            <TouchableOpacity
              style={{ marginTop: 8, width: '100%', backgroundColor: '#e5e7eb', borderRadius: 8, padding: 14, alignItems: 'center' }}
              onPress={() => setShowApiInfoModal(false)}
            >
              <Text style={{ color: '#111', fontWeight: '700', fontSize: 15 }}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
                      ...
                      {organizations.map(org => (
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            setSearchSubinventory(org.subinventories?.[0]?.code || '');
                            setShowOnhandOrgDropdown(false);
                          }}
                        >
                          <Text style={{ fontSize: 14, fontWeight: '600', color: COLORS.text }}>{org.warehouse}</Text>
                          <Text style={{ fontSize: 11, color: '#888' }}>{org.warehouse_code}{org.subinventories?.length > 0 ? ` • ${org.subinventories.map(s => s.code).join(', ')}` : ''}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  </View>
  ...
  if (currentScreen === 'LocatorTransferDetail' && locatorTransfer_selectedItem) {
    const handleTransfer = async () => {
      ...
      try {
        // Call APEX Subinventory Transfer API
        const transferUrl = 'https://g827cd88c3cfc03-mitsumioracledb.adb.me-dubai-1.oraclecloudapps.com/ords/test/INVENTORY/submitSubinventoryTransfer';

        const transferPayload = {
          warehouse_code: locatorTransfer_selectedWarehouse.code,
          from_subinventory_code: locatorTransfer_selectedSubinventory.code,
          from_locator_id: locatorTransfer_selectedSourceLocator.code,
          to_subinventory_code: locatorTransfer_selectedSubinventory.code, // Same subinventory for now
          to_locator_id: locatorTransfer_selectedDestLocator.code,
          item_number: locatorTransfer_selectedItem.itemNumber,
          transfer_quantity: qty,
          uom_code: locatorTransfer_selectedItem.uomCode
        };
        ...
        const refNum = result.transfer_id || result.reference_number || `TRN-${Date.now()}`;
              ...
              <ScrollView>
                {organizations.map(org => (
                  <View key={org.warehouse} style={{ marginBottom: 10 }}>
                    ...
                    {/* Subinventory List with LID column */}
                    {/* Column header */}
                    {org.subinventories && org.subinventories.length > 0 && (
                      <View style={{ flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 4, backgroundColor: '#e5e7eb' }}>
                        <Text style={{ fontSize: 10, fontWeight: '700', color: '#6b7280', flex: 1 }}>SUBINVENTORY</Text>
                        <Text style={{ fontSize: 10, fontWeight: '700', color: '#6b7280', width: 80, textAlign: 'center' }}>LID</Text>
                      </View>
                    )}
                    {org.subinventories && org.subinventories.length > 0 ? (
                      org.subinventories.map(sub => {
                        const isSelected = locatorSelectedOrg?.warehouse === org.warehouse && locatorSelectedSub === sub.code;
                        const hasLid = !!sub.locator_id;
                        return (
                          <TouchableOpacity
                            key={sub.code}
                          ...
                          >
                            ...
                            <View style={{ width: 80, alignItems: 'center' }}>
                              {hasLid ? (
                                <Text style={{ fontSize: 10, color: '#059669', fontWeight: '700', fontFamily: 'monospace' }} numberOfLines={1}>
                                  ✓ {sub.locator_id.toString().slice(-6)}
                                </Text>
                              ) : (
                                <Text style={{ fontSize: 14, color: '#ef4444', fontWeight: '700' }}>✗</Text>
                              )}
                            </View>
                            {isSelected && <Text style={{ fontSize: 11, color: '#059669', fontWeight: '700', marginLeft: 4 }}>✓</Text>}
                          </TouchableOpacity>
                        );
                      })
                    ) : (
        ...
        {stockLocatorsTab === 'map' && hasEnoughFilters && (
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
            ...
            {(() => {
              ...
              const areaElements = Object.entries(areaGroups).sort().map(([area, data]) => (
